...
int findPeakElement(vector<int> &arr) {
    ...
    while(low <= high){
        ...
        bool leftSmall  = (mid == 0 || arr[mid] > arr[mid - 1]);
        bool rightSmall = (mid == n-1 || arr[mid] > arr[mid + 1]);
        
        if(leftSmall && rightSmall) return mid;  // peak found
        else if(mid > 0 && arr[mid-1] > arr[mid]) high = mid - 1;  // go left
