...
public:
    int earliestFinishTime(vector<int>& landStartTime, vector<int>& landDuration, vector<int>& waterStartTime, vector<int>& waterDuration) {
        int land = INT_MAX , w_l = INT_MAX, l_w = INT_MAX, water = INT_MAX;
        ...
        for(int i = 0; i < m; i++){
            ...
            l_w = min(l_w , max(land,waterStartTime[i]) + waterDuration[i]);
        }
        for(int i = 0; i < n; i++){
            w_l = min(w_l , max(water,landStartTime[i]) + landDuration[i]);
        }
